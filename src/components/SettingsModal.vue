...
import { AVAILABLE_THEMES, getThemeById } from '@/common/editor/themes'
...
// 编辑器主题选项（从主题定义中生成）
const editorThemeOptions = AVAILABLE_THEMES.map((theme) => ({
  value: theme.id,
  label: theme.name,
...
// 深色主题选项
const darkThemeOptions = computed(() => editorThemeOptions.filter((t) => t.isDark))

// 亮色主题选项
const lightThemeOptions = computed(() => editorThemeOptions.filter((t) => !t.isDark))
...
// 监听编辑器主题变化，实时预览
watch(
  () => localSettings.editorTheme,
  (newTheme) => {
    // 实时更新 store 以预览效果
    settingStore.settings.editorTheme = newTheme
    // 根据主题自动设置系统主题模式
    const theme = getThemeById(newTheme)
    appStore.setTheme(theme.isDark ? 'dark' : 'light')

    // 自动更新偏好主题（用于快捷切换）
    if (theme.isDark) {
      localSettings.preferredDarkTheme = newTheme
    } else {
      localSettings.preferredLightTheme = newTheme
    }
  },
)
          ...
          <section class="settings-section">
            ...
            <div class="setting-item">
              ...
              <div class="theme-group">
                <span class="theme-group-label">🌙 深色</span>
                <div class="theme-grid">
                  <button v-for="option in darkThemeOptions" :key="option.value"
                    :class="['theme-card', { active: localSettings.editorTheme === option.value }]"
                    @click="localSettings.editorTheme = option.value">
                    <div class="theme-preview" :style="{
                      background: getThemeById(option.value).colors.background,
                      color: getThemeById(option.value).colors.foreground,
                      borderColor: getThemeById(option.value).colors.accent,
                    }">
                      <span class="preview-heading"
                        :style="{ color: getThemeById(option.value).colors.heading }">Aa</span>
                    </div>
                    <span class="theme-name">{{ option.label }}</span>
                    <span v-if="localSettings.preferredDarkTheme === option.value" class="preferred-badge"
                      title="快捷切换默认主题">⚡</span>
                  </button>
                </div>
              </div>
              ...
              <!-- 亮色主题 -->
              <div class="theme-group">
                <span class="theme-group-label">☀️ 亮色</span>
                <div class="theme-grid">
                  <button v-for="option in lightThemeOptions" :key="option.value"
                    :class="['theme-card', { active: localSettings.editorTheme === option.value }]"
                    @click="localSettings.editorTheme = option.value">
                    <div class="theme-preview" :style="{
                      background: getThemeById(option.value).colors.background,
                    ...
                    <span class="theme-name">{{ option.label }}</span>
                    <span v-if="localSettings.preferredLightTheme === option.value" class="preferred-badge"
                      title="快捷切换默认主题">⚡</span>
                  </button>
                </div>
              </div>
          ...
          <section class="settings-section">
            ...
            <div v-if="localSettings.autoSave" class="setting-item">
              ...
              <div class="range-container">
                ...
                <input type="range" v-model.number="localSettings.autoSaveDelay" min="500" max="5000" step="500"
                  class="range-input" />
